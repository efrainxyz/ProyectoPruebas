...
/**
 * Servlet implementation class ServletDetalleOrdenCompra
 */
@WebServlet("/DetalleOrdenCompra")
public class ServletDetalleOrdenCompra extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ServletDetalleOrdenCompra() {
        super();
			...
			try {
				if(user.getRol().getNomTipo().equals("EncargadoCompras")){
					try {
						...
						ProveedorBean proveedor=proveedordao.buscarProveedor2((detalleorden.get(0).getOrden().getProveedor_idProveedor()));
						if(proveedor!=null){
					...
					request.getRequestDispatcher("/EncargadoCompras/DetalleOrden.jsp").forward(request, response);
				}else{
